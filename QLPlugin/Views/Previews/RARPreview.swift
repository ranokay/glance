...
class RARPreview: Preview {
	...
	func createPreviewVC(file: File) async throws -> PreviewVC {
		...
		let payload = try await PreviewExecutor.run {
			try PreviewCoreBridge.scanRAR(at: fileURL)
		}
	...
	private func makeFileTree(from entries: [ArchivePreviewEntry]) throws -> FileTree {
		...
		for entry in entries {
			...
			} catch let error as RARPreviewError {
				throw error
			} catch {
	...
	private func checkedInt(_ value: UInt64) throws -> Int {
		guard value <= UInt64(Int.max) else {
			throw RARPreviewError.metadataSizeLimitExceeded
		}
		return Int(value)
	}
}

private enum RARPreviewError: LocalizedError {
